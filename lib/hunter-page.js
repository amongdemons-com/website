...
function injectHunterData(template, payload) {
  ...
  return replaceFirst(
    template,
    '<!--HUNTER_DATA-->',
    `<script id="hunterInitialData" type="application/json">${json}</script>`
  );
}
...
function buildHunterMeta(profile, rawUsername) {
  ...
  const image = `${CANONICAL_ORIGIN}/api/og/hunter/${encodeURIComponent(username)}.png?v=4`;
...
function buildDescription(profile) {
  const hunter = profile.hunter || {};
  const parts = [
    ...(profile.ranked?.division ? [`${profile.ranked.division} rank`] : []),
...
function injectHunterContent(template, profile) {
  ...
  const ranked = profile.ranked || null;
  let html = template;
  ...
  html = replaceFirst(
    html,
    '<span id="hunterBadges" class="player-badges player-badges--hunter" aria-label="Hunter badges"></span>',
    renderServerPlayerBadges(profile.badges)
  );
  html = replaceFirst(
    html,
    '<p id="hunterSubline">Loading public record...</p>',
    `<p id="hunterSubline">${ranked ? `${renderRankDivisionText(ranked.division)} &middot; ` : ''}Level ${formatNumber(hunter.level)} &middot; ${formatNumber(hunter.pvpWins)}-${formatNumber(hunter.pvpLosses)}</p>`
  );
  html = replaceFirst(
    html,
  ...
  html = replaceFirst(
    html,
    '<span class="hunter-stat-value" data-hunter-stat="ranked">Unranked</span>',
    `<span class="hunter-stat-value" data-hunter-stat="ranked">${ranked ? `#${formatNumber(ranked.rank)} ${escapeHtml(ranked.division)}` : 'Unranked'}</span>`
  );
  html = replaceFirst(
    html,
...
function renderServerPlayerBadges(badges = []) {
  const entries = (Array.isArray(badges) ? badges : []).filter((badge) => (
    badge && badge.key && badge.name && badge.description
  ));
  return `<span id="hunterBadges" class="player-badges player-badges--hunter" aria-label="Hunter badges">
    ${entries.map((badge) => {
      const key = String(badge.key).toLowerCase().replace(/[^a-z0-9_-]+/g, '-');
      const label = `${badge.name}. ${badge.description}`;
      const icon = String(badge.icon || 'shield').toLowerCase().replace(/[^a-z0-9-]+/g, '');
      const actionLabel = String(badge.action?.label || '').trim();
      const actionHref = String(badge.action?.href || '').trim();
      const hasAction = Boolean(actionLabel && /^https:\/\//i.test(actionHref));
      const actionHtml = hasAction
        ? `<a class="player-badge-tooltip-action" href="${escapeAttribute(actionHref)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeAttribute(`${actionLabel} for ${badge.name} (opens in a new tab)`)}"><span>${escapeHtml(actionLabel)}</span><i data-lucide="arrow-right"></i></a>`
        : '';
      return `<span class="player-badge player-badge--${escapeAttribute(key)}${hasAction ? ' player-badge--has-action' : ''}" tabindex="0" aria-label="${escapeAttribute(label)}">
        <span class="player-badge-mark" aria-hidden="true"><i data-lucide="${escapeAttribute(icon)}"></i></span>
        <span class="player-badge-tooltip${hasAction ? ' player-badge-tooltip--action' : ''}" role="${hasAction ? 'group' : 'tooltip'}"${hasAction ? ` aria-label="${escapeAttribute(`${badge.name} badge details`)}"` : ''}>
...
function renderRankDivisionText(division) {
  ...
  const slug = label.toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');
  return `<span class="rank-division-text rank-division-text--${escapeAttribute(slug)}">${escapeHtml(label)}</span>`;
}
