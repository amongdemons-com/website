...
async function renderHunterOgPng(profile, rawUsername) {
  ...
  return sharp(Buffer.from(svg))
    .composite(createTextOverlays(view))
    .png()
    .toBuffer();
}
...
<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}">
  ...
  <g transform="translate(72 92)">
    ...
    <text x="2" y="160" fill="#edf5f2" fill-opacity="0.78" font-size="27" font-weight="800">${escapeXml(subtitle)}</text>
  </g>
  ...
  <text x="933" y="541" text-anchor="middle" fill="#a1d4c9" font-size="18" font-weight="900" letter-spacing="3">${found ? 'PROFILE DEMON' : 'BRANDED FALLBACK'}</text>
...
function createTextOverlays(view) {
  const overlays = [
    ...
    textOverlay(view.subtitle, 74, 233, {
      color: '#c2cbc9',
      size: 27,
      weight: '800'
    }),
    ...
    textOverlay(renderDemonNameMarkup(view), 754, 483, {
      align: 'center',
      markup: true,
      size: 34,
      weight: '900',
    ...
    textOverlay(view.found ? spacedBrandText('PROFILE DEMON') : spacedBrandText('BRANDED FALLBACK'), 754, 527, {
      align: 'center',
      color: '#a1d4c9',
      fontFamily: DISPLAY_FONT_FAMILY,
      fontfile: DISPLAY_FONT_PATH,
