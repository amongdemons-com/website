...
async function renderHunterOgPng(profile, rawUsername) {
  const svg = await renderHunterOgSvg(profile, rawUsername);
...
async function renderHunterOgSvg(profile, rawUsername) {
  ...
  const subtitle = found
    ? `Level ${formatNumber(hunter.level)} | ${formatNumber(hunter.pvpWins)}-${formatNumber(hunter.pvpLosses)} PvP`
...
<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}">
  ...
  <g transform="translate(72 92)">
    <text x="0" y="0" fill="#e8c76a" font-family="Arial, Helvetica, sans-serif" font-size="25" font-weight="900" letter-spacing="5">AMONG DEMONS</text>
    <text x="0" y="104" fill="#fff8dc" font-family="Georgia, 'Times New Roman', serif" font-size="${usernameFontSize}" font-weight="900"${getTextFitAttributes(displayUsername, 618)}>${escapeXml(displayUsername)}</text>
    <text x="2" y="160" fill="#edf5f2" fill-opacity="0.78" font-family="Arial, Helvetica, sans-serif" font-size="27" font-weight="800">${escapeXml(subtitle)}</text>
  </g>

  ${stats.map((stat, index) => renderStatCard(stat, index)).join('\n  ')}
  ...
  <text x="933" y="510" text-anchor="middle" fill="#fff8dc" font-family="Georgia, 'Times New Roman', serif" font-size="34" font-weight="900">
  ...
  <text x="933" y="541" text-anchor="middle" fill="#a1d4c9" font-family="Arial, Helvetica, sans-serif" font-size="18" font-weight="900" letter-spacing="3">${found ? 'PROFILE DEMON' : 'BRANDED FALLBACK'}</text>

  <g transform="translate(72 560)">
    <text x="0" y="0" fill="#edf5f2" fill-opacity="0.82" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="800">amongdemons.com</text>
  </g>
</svg>`;
}
...
function renderStatCard(stat, index) {
  ...
  return `<g transform="translate(${x} ${y})">
    ...
    <text x="20" y="34" fill="#f4d675" font-family="Arial, Helvetica, sans-serif" font-size="30" font-weight="900">${escapeXml(truncateText(stat.value, 15))}</text>
    <text x="20" y="59" fill="#edf5f2" fill-opacity="0.58" font-family="Arial, Helvetica, sans-serif" font-size="15" font-weight="900" letter-spacing="2">${escapeXml(stat.label.toUpperCase())}</text>
  </g>`;
}
