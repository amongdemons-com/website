...
const DEMONS_IMAGE_PATH = '/app/images/demons/boof-nitza-common.png';
      ...
      <section class="seo-hero seo-home-hero">
        <div class="seo-hero-copy">
          ...
          <div class="seo-action-row">
            <a class="btn btn-primary btn-lg seo-enter-dungeon-button" href="/dungeon">Enter Dungeon</a>
...
function renderSitemap(catalog) {
  const sortedCatalog = sortCatalog(catalog);
  const typeIds = [...new Set(sortedCatalog.map((demon) => Number(demon.type)))].sort((a, b) => a - b);
  const entries = [
    {
      loc: canonicalUrl('/'),
      priority: '1.0',
    ...
    ...typeIds.map((typeId) => {
      const firstDemon = sortedCatalog.find((demon) => Number(demon.type) === typeId);
      return {
        loc: canonicalUrl(`/demons/type/${typeId}`),
        priority: '0.6',
        changefreq: 'monthly',
        images: firstDemon ? [imageEntry(getDemonImagePath(firstDemon), `${getDemonName(firstDemon)} demon type artwork`)] : []
      };
    }),
      ...
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
          ...
          <div class="nav-account d-none" data-nav-account>
            ...
            <span class="nav-player" title="Signed in player">
  ...
  return `
    <footer class="py-4 border-top">
      <div class="container text-center text-body-secondary">
        <p class="mb-0">&copy; 2026 Among Demons. All rights reserved.</p>
      </div>
    </footer>
  `;
}
