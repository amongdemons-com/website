...
const { getDemonImageUrl } = require('../public/api/lib/demon-images');
...
const DEFAULT_IMAGE_PATH = '/app/images/assets/background/amongdemons_home.png?v=art-102485fdb26d';
const DEFAULT_IMAGE_PATH_LOGO = '/app/images/assets/background/amongdemons_home_logo.png?v=art-102485fdb26d';
const DUNGEON_IMAGE_PATH = '/app/images/assets/background/amongdemons_dungeon_cartoon_v2.png?v=art-102485fdb26d';
const DEMONS_IMAGE_PATH = '/app/images/demons/1.png?v=art-102485fdb26d';
...
const EQUIPMENT_RARITIES = ['common', 'uncommon', 'rare', 'epic', 'legendary', 'mythic'];
...
const EQUIPMENT_SHORT_VALUE_LABELS = {
  allyDamageMaxHpPercent: 'scorch',
  damageBuffPercent: 'dmg buff',
  damagePercent: 'dmg',
  damagePercentPerDeadAlly: 'dmg/fallen',
  damagePercentPerRole: 'dmg/role',
  damagePercentPerSlot: 'dmg/slot',
  damageTakenPercent: 'taken',
  damageTakenPercentPerDeadAlly: 'taken/fallen',
  durationPercent: 'duration',
  durationTicks: 'duration',
  healMaxHpPercent: 'heal',
  healingPercent: 'healing',
  healingReductionPercent: 'enemy healing',
  initialHpPercent: 'start HP',
  intervalTicks: 'interval',
  invulnerableTicks: 'invuln',
  maxHpPercent: 'HP',
  onKillDamagePercent: 'on-kill dmg',
  otherAlliesDamagePercent: 'other dmg',
  reflectDamagePercent: 'reflect',
  reflectionCritChancePercent: 'reflect crit',
  reflectionCritDamagePercent: 'crit dmg',
  regenBelowHpBonusPercent: 'low-HP regen',
  regenMaxHpPercent: 'regen',
  resurrectHpPercent: 'revive HP',
  sacrificeShieldPercent: 'sacrifice shield',
  sharePercent: 'shared stats',
  shieldMaxHpPercent: 'shield',
  speedPercent: 'AS',
  speedPercentPerDeadAlly: 'AS/fallen',
  teamCapacity: 'capacity',
  thresholdPercent: 'threshold',
  tickDamagePercent: 'poison tick'
};
...
function getDemonImagePath(demon) {
  return `/app/images/demons/${getDemonSlug(demon)}.png?v=art-102485fdb26d`;
}
    ...
    main: `
      ...
      <section class="seo-section">
        ...
        <div class="seo-media-grid">
          ...
          ${renderPressAsset('/app/images/assets/background/amongdemons_collection.png?v=art-102485fdb26d', 'Collection artwork', '1600 × 900 PNG')}
        </div>
        <div class="seo-action-row">
...
function renderDemonsPage(catalog) {
  ...
  return renderDocument({
    ...
    jsonLd,
    entryScrollTarget: 'guide-content',
    main: `
      ...
      <section id="guide-content" class="seo-section">
        <div class="seo-section-heading">
...
function renderItemsPage() {
  const items = equipmentCatalog;
  const title = 'Equipment Items | Among Demons Item Guide';
  const description = 'Browse every Among Demons equipment item, including its slot, tags, effect, and scaling from Common to Mythic rarity.';
  const imagePath = DEFAULT_IMAGE_PATH_LOGO;
  const imageAlt = 'Among Demons equipment item guide';
  const jsonLd = buildBaseJsonLd({
    title,
    description,
    canonicalPath: '/items',
  ...
  return renderDocument({
    ...
    imagePath,
    imageAlt,
    bodyClass: 'seo-page seo-items-page',
    jsonLd,
    entryScrollTarget: 'guide-content',
    main: `
      <section class="seo-hero seo-items-hero">
        <div class="seo-hero-copy">
          <span class="seo-kicker">Equipment item guide</span>
          <h1>Items</h1>
          <p>Compare every piece of equipment in Among Demons, from starter weapons to Mythic relics that reshape your team's battle plan.</p>
        </div>
      </section>
      ...
      <section id="guide-content" class="seo-section">
        <div class="seo-section-heading">
          ...
          <p>${items.length} items across five equipment slots, with effects shown at every rarity.</p>
        </div>
        <div class="seo-items-legend" aria-label="Rarity key">
          ${EQUIPMENT_RARITIES.map((rarity) => `<span class="seo-items-legend-item seo-items-rarity--${rarity}">${renderEquipmentRarityBadge(rarity)}</span>`).join('')}
        </div>
        <div class="seo-items-list" role="list" aria-label="Among Demons equipment items">
          ${items.map(renderEquipmentItemRow).join('')}
        </div>
      </section>
    `
  });
...
function renderEquipmentItemRow(item) {
  const slot = formatLabel(item.slot);
  const icon = item.imageUrl
    ? `<img src="${escapeHtml(item.imageUrl)}" alt="" width="36" height="36" loading="lazy" decoding="async">`
    : `<i data-lucide="${escapeHtml(item.icon || item.slot)}" aria-hidden="true"></i>`;
  const tags = (item.tags || [])
    .map((tag) => `<span>${escapeHtml(formatLabel(tag))}</span>`)
    .join('');
  const effectType = formatLabel(item.effect?.type || 'effect');
  const targetRoles = item.effect?.targetRoles || item.targetRoles;
  const targetTypeIds = item.effect?.targetTypeIds || item.targetTypeIds;
  const condition = item.effect?.condition || item.condition;
  const effectDetails = [
    Array.isArray(targetRoles) && targetRoles.length
      ? `Targets ${targetRoles.map(formatLabel).join(', ')}`
      : '',
    Array.isArray(targetTypeIds) && targetTypeIds.length
      ? `Demon types ${targetTypeIds.map((typeId) => TRAIT_LABELS_BY_TYPE[typeId] || typeId).join(', ')}`
      : '',
    condition ? formatEquipmentCondition(condition) : ''
  ].filter(Boolean);

  return `
    <article class="seo-item-row" role="listitem">
      <div class="seo-items-art" aria-hidden="true">${icon}</div>
      <div class="seo-items-primary">
        <div class="seo-items-name-content">
          <span>${escapeHtml(item.name)}</span>
        </div>
        <div class="seo-items-primary-meta">
          <span class="seo-items-slot">${escapeHtml(slot)}</span>
          <div class="seo-items-tags">${tags}</div>
        </div>
      </div>
      <div class="seo-items-effect">
        <strong>${escapeHtml(item.description || effectType)}</strong>
        <span class="seo-items-effect-type">${escapeHtml(effectType)}</span>
        ${effectDetails.length ? `<span class="seo-items-effect-details">${escapeHtml(effectDetails.join(' · '))}</span>` : ''}
      </div>
      ${renderEquipmentRarityScale(item)}
    </article>
  `;
}

function renderEquipmentRarityScale(item) {
  return `
    <div class="seo-items-rarity-scale" aria-label="${escapeHtml(item.name)} rarity scaling">
      ${EQUIPMENT_RARITIES.map((rarity) => renderEquipmentRarityChip(item, rarity)).join('')}
    </div>
  `;
}

function renderEquipmentRarityChip(item, rarity) {
  const values = item.effect?.values?.[rarity] || {};
  const fullValues = Object.entries(values)
    .map(([key, value]) => `${EQUIPMENT_VALUE_LABELS[key] || formatEquipmentLabel(key)} ${formatEquipmentValue(key, value)}`)
    .join(', ');
  const compactValues = Object.entries(values)
    .map(([key, value]) => formatEquipmentCompactValue(key, value))
    .join(' · ');
  const label = `${capitalize(rarity)}: ${fullValues || 'No scaling data'}`;
  return `<span class="seo-items-rarity-chip seo-items-rarity--${rarity}" title="${escapeHtml(label)}" aria-label="${escapeHtml(label)}">${renderEquipmentRarityBadge(rarity, { showLabel: false })}<span class="seo-items-rarity-value">${escapeHtml(compactValues || '—')}</span></span>`;
}

function renderEquipmentRarityBadge(rarity, options = {}) {
  const label = capitalize(rarity);
  const labelMarkup = options.showLabel === false
    ? ''
    : `<span class="seo-items-rarity-name">${escapeHtml(label)}</span>`;
  return `<span class="seo-items-rarity-badge"><span class="dungeon-demon-rarity-gem dungeon-demon-rarity-gem--${escapeHtml(rarity)}" aria-hidden="true"></span>${labelMarkup}</span>`;
}

function formatEquipmentCondition(condition = {}) {
  return Object.entries(condition)
    .map(([key, value]) => `${formatEquipmentLabel(key)}: ${formatLabel(value)}`)
    .join(', ');
}

function formatEquipmentLabel(value) {
  return formatLabel(String(value || '').replace(/([a-z])([A-Z])/g, '$1 $2'));
}
...
function formatEquipmentCompactValue(key, value) {
  const shortLabel = EQUIPMENT_SHORT_VALUE_LABELS[key];
  if (shortLabel) return `${formatEquipmentValue(key, value)} ${shortLabel}`;
  if (['selector', 'target', 'targeting'].includes(key)) return formatLabel(value);
  return `${formatEquipmentValue(key, value)} ${EQUIPMENT_VALUE_LABELS[key] || formatEquipmentLabel(key)}`;
}
...
const WORLD_EVENTS_GUIDE = [
  ...
  },
  {
    ...
    blurb: 'Offer Souls at this fixed altar for a nine-floor battle against The Anomaly, which channels all eleven demon types and always retaliates with Thorns. Every defeated Anomaly makes its own 25% Mythic reward roll, with completed Mythic collections converting successful rolls into five hunter levels.',
      ...
      <section class="seo-boss-priority" style="--rarity-color: ${escapeHtml(getRarityColor(keyDemon?.rarity || 'mythic'))}">
        ...
        <div class="seo-boss-priority-grid">
          ...
          <aside class="seo-boss-summary-panel">
            ...
            <div class="seo-action-row">
              <a class="btn btn-primary" href="/world">Challenge</a>
...
function renderSitemap(catalog, bossCatalog = []) {
  ...
  const entries = [
    ...
    {
      loc: canonicalUrl('/items'),
      priority: '0.8',
      changefreq: 'weekly',
      images: [imageEntry(DEFAULT_IMAGE_PATH_LOGO, 'Among Demons equipment item guide artwork')]
    },
    {
    ...
    },
    {
      ...
      priority: '0.85',
      changefreq: 'weekly',
      images: [imageEntry('/app/images/assets/background/amongdemons_collection.png?v=art-102485fdb26d', 'Among Demons demon collection checklist artwork')]
    },
    {
    ...
    },
    {
      ...
      priority: '0.5',
      changefreq: 'daily',
      images: [imageEntry('/app/images/assets/background/amongdemons_rankings.png?v=art-102485fdb26d', 'Among Demons dungeon rankings artwork')]
    },
    {
...
<html lang="en" data-bs-theme="dark">
  ...
  <body class="${escapeHtml(bodyClass)}"${entryScrollTarget ? ` data-entry-scroll-target="${escapeHtml(entryScrollTarget)}"` : ''}>
    ...
    ${entryScrollTarget ? `<script>
      (() => {
        if (window.location.hash || window.scrollY > 0) return;
        const targetId = document.body.dataset.entryScrollTarget;
        ...
        <div class="collapse navbar-collapse" id="publicGameNav">
          ...
          <div class="nav-account d-none" data-nav-account>
            ...
            <a class="nav-player" href="/camp" title="Go to camp">
              <span class="nav-player-avatar" aria-hidden="true"><img src="/app/images/demons/map/1.webp?v=art-102485fdb26d" alt="" width="34" height="34" loading="eager" data-nav-profile-image></span>
            ...
            <div class="${classes}" data-formation-slot="${assignment?.slot ?? originalSlot}" role="listitem" aria-label="Boss team slot ${(assignment?.slot ?? originalSlot) + 1}">
              <div class="formation-slot-cards">
                ${assignment
                  ...
                  : `<div class="formation-empty formation-empty-${position}" aria-hidden="true" data-slot-number="${originalSlot + 1}">${renderFormationSlotPlaceholder(position)}</div>`}
              </div>
            </div>`;
        }).join('')}
      </div>
...
function renderBossFormationMember(member) {
  ...
  const rarityKey = getDemonRarityKey(member.rarity);
  ...
  return `
    <${tag} class="dungeon-demon-card hunter-team-card" style="--rarity-color: ${escapeHtml(getRarityColor(member.rarity))}"${href}>
      <div class="dungeon-demon-card-image" aria-label="${escapeHtml(capitalize(member.rarity))} rarity">
        <img src="${escapeHtml(getDemonImageUrl(member, 'portrait') || LOGO_PATH)}" alt="${escapeHtml(`${title} in the boss formation`)}" width="${DEMON_IMAGE_WIDTH}" height="${DEMON_IMAGE_HEIGHT}" loading="eager" decoding="async" draggable="false" onerror="this.onerror=null;this.src='${LOGO_PATH}';">
        <span class="dungeon-demon-rarity-gem dungeon-demon-rarity-gem--${rarityKey}" aria-hidden="true"></span>
      </div>
...
function renderFormationSlotPlaceholder(position) {
  return position === 'front'
    ? '<span class="formation-slot-placeholder-icon" aria-hidden="true"><i data-lucide="shield"></i></span>'
    : '<span class="formation-slot-placeholder-icon" aria-hidden="true"><i data-lucide="bow-arrow"></i></span>';
}
...
function renderDemonImage(demon, options = {}) {
  ...
  const imageUrl = getDemonImageUrl({ sourceDemonId: demon.id }, 'portrait');
  return `<img src="${imageUrl}" alt="${escapeHtml(alt)}" width="${DEMON_IMAGE_WIDTH}" height="${DEMON_IMAGE_HEIGHT}" loading="${options.loading || 'lazy'}" decoding="async" onerror="this.onerror=null;this.src='${LOGO_PATH}';">`;
}

function renderBossImage(boss, options = {}) {
  const title = boss.title || formatLabel(boss.id);
  const alt = options.alt || `${title}, a world boss in Among Demons`;
  const rarityKey = getDemonRarityKey(boss.keyDemon?.rarity || 'mythic');
  return `<img src="${escapeHtml(getDemonImageUrl(getBossImagePath(boss), 'portrait'))}" alt="${escapeHtml(alt)}" width="${DEMON_IMAGE_WIDTH}" height="${DEMON_IMAGE_HEIGHT}" loading="${options.loading || 'lazy'}" decoding="async" onerror="this.onerror=null;this.src='${LOGO_PATH}';">
    <span class="dungeon-demon-rarity-gem dungeon-demon-rarity-gem--${rarityKey} boss-key-demon-gem" role="img" aria-label="${capitalize(rarityKey)} rarity"></span>`;
}

function getDemonRarityKey(rarity) {
  const key = String(rarity || '').trim().toLowerCase();
  return RARITY_ORDER.includes(key) ? key : 'common';
}
