...
const { BASELINE, groundImage } = require('./demon-grounding');
...
async function main() {
  ...
  for (const name of approved) {
    ...
    const grounded = groundImage(data, info.width, info.height);
    records.push({ id: parseInt(name), sourceSize: [info.width, info.height], bounds: grounded.bounds, translation: grounded.translation, baseline: grounded.baseline });
    for (const [variant, size] of [['portrait', 512], ['map', 256]]) {
      const image = await sharp(grounded.data, { raw: { width: info.width, height: info.height, channels: 4 } })
  ...
  fs.writeFileSync(path.join(__dirname, '../docs/art/demon-grounding.json'), JSON.stringify({
    baselineFraction: BASELINE,
    method: 'Integer canvas translation only before downsampling. Master drawings and relative scale unchanged. Bounds include detached features and ignore components smaller than 16 alpha pixels; a 2px edge margin preserves antialiasing.',
    records
  }, null, 2) + '\n');
