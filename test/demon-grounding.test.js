...
const { BASELINE, findContentBounds, groundImage } = require('../scripts/demon-grounding');
...
test('empty and oversized artwork fail instead of being silently clipped', () => {
  assert.throws(() => groundImage(Buffer.alloc(100 * 100 * 4), 100, 100), /No substantive/);
  const data = Buffer.alloc(100 * 100 * 4);
  rectangle(data, 100, 20, 0, 40, 100);
  assert.throws(() => groundImage(data, 100, 100), /clip/);
});
