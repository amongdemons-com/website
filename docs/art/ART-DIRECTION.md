...
- Dungeon: use the wide Dungeon environment as one centered cover image. Do not tile or repeat the square blue rock texture. Use the original live-site layout: framed and padded 3×3 player/enemy grids, visible empty slots, original arena spacing and the established responsive rearrangement.
...
Live Dungeon formations retain the original framed 3×3 grids and slot interactions. Occupied cards use their approved per-type swirl backdrop, rounded 7px artwork corners and a light neutral cell boundary; neither the card nor the structural slot boundary is rarity-colored. Keep compact combat stats over the same full-width bottom fade used by hand cards. Formation HP bars have no border or box shadow, and demon images do not use a drop-shadow filter. Floating combat amounts use at least twice the former 1.22rem size. Empty slots, Collection add controls and active drag targets use the original live-site treatment.
