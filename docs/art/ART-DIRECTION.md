...
Show the complete transparent demon using `object-fit: contain`, centered with no image zoom. The image layer fills the entire card, with compact text and combat stats overlaid at the bottom. Do not reserve space outside the art for a separate stats panel: the user rejected that presentation because it shortened the artwork. Within the overlay, put the HP bar above one footer row: attack and speed on the left, HP value and icon on the right. The card body has `1em` top padding at all viewport sizes, and `.dungeon-demon-card-image` has `border-radius: 0`. Preserve the existing compact phone-hand separator in place of its hidden HP bar. Horns, wings, tails, claws and the actual poison puddle must remain within the image frame. Apply this on desktop, portrait phones, compressed formations, hands, collection, rewards and detail views. Collection cards fill their available grid cells. Do not restore the historical 1.3× desktop / 1.48× phone scaling or `cover` crop rules.
