...
Demon direction approved 2026-08-27. This is the production direction for the website artwork branch. Card placement and texture direction updated 2026-08-28 for user review.
...
Show the complete transparent demon using `object-fit: contain`, centered with no image zoom. The image layer fills the entire card, with compact text and combat stats overlaid at the bottom. Do not reserve a separate stats row: the user rejected that presentation because it shortened the artwork. Horns, wings, tails, claws and the actual poison puddle must remain within the image frame. Apply this on desktop, portrait phones, compressed formations, hands, collection, rewards and detail views. Collection cards fill their available grid cells. Do not restore the historical 1.3× desktop / 1.48× phone scaling or `cover` crop rules.

The shared card backdrop is bright jade stone with restrained pale yellow-green moss/lichen, drawn from the collection and summon environments. Keep broad matte color planes, shallow scuffs and a calm center. Background shape boundaries use nearby colors, never black outlines: the demon's near-black contour must remain the strongest edge. Aim for roughly 70% quiet color and two simplified material textures, concentrated toward the edges. No scene horizon, platform, cast shadow, spotlight, tiny repeated marks or scenery props. Keep detailed environments on the page, not inside every card. The reusable background stays in a separate CSS layer, never baked into transparent sprites.

The earlier forest was too busy; the subsequent muted teal field was too empty. Do not revert to either. A warm amber limestone alternative is retained in `backdrop-studies/`, but use one consistent shared backdrop across species and rarities so background color does not become an unintended rarity signal. Research, selection reasoning and exact prompts are in `CARD-BACKDROP-RESEARCH.md` and `card-backdrop-texture-prompts.json`.
...
Demons use `scripts/extract-demon-backgrounds.js` only for deterministic extraction from the approved matte concepts. It changes transparency and antialiased edge color, preserving interior RGB. Do not use it on arbitrary painted images or future assets with different backgrounds without visual review. The additional drawing edits in this integration are the three localized Vi'Zel repairs in `scripts/touch-up-vizel-eyes.js` and Tor Tza's common pupil-to-eyelid bridge in `scripts/touch-up-tor-tza-eye.js`; they preserve every alpha value and every pixel outside the recorded patch rectangles. Display centering changes placement only, not anatomy or painted details.
