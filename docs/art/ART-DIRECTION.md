...
- Dungeon: use the wide Dungeon environment as one centered cover image. Do not tile or repeat the square blue rock texture. Use the original live-site layout: framed and padded 3×3 player/enemy grids, visible empty slots, original arena spacing and the established responsive rearrangement. Render the grid shell, cells and placeholders as clean opaque cartoon color blocks with crisp borders—no gradients, glows, shadows or translucent washes on the grid UI.
...
Live Dungeon formations retain the original framed 3×3 grids and slot interactions. The grid shell and every slot use solid, opaque team/enemy color blocks with crisp borders; grid surfaces, placeholders and drag states have no gradients, glows, shadows or translucent washes. Apply that same flat, opaque treatment to the hand tray, its tabs and placeholders, the replay/log box, and battle playback/speed controls. Occupied cards use their approved per-type swirl backdrop, rounded 7px artwork corners and a clean side-colored cell boundary; neither the card nor the structural slot boundary is rarity-colored. Keep compact combat stats over the same full-width bottom fade used by hand cards. Formation HP bars have no border or box shadow, and demon images do not use a drop-shadow filter. Floating combat amounts use at least twice the former 1.22rem size. Collection add controls and active drag targets follow the same flat opaque grid treatment.

 Card backdrops now use the approved colorful cartoon swirl assignments. Keep broad matte planes, confident color boundaries, the existing restrained texture and roughly 70% quiet center. Types 4, 6 and 11 reuse type 2's background; types 5, 7 and 9 reuse type 1's. Do not return to faded pastel washes or add extra curls, black background ink, glow or scenery. The demon's near-black outline remains the strongest edge. No horizon, floor, platform, puddle, cast shadow, particles, symbols or props. Keep detailed environments on the page, not inside every card. Backdrops remain separate CSS layers, never baked into transparent sprites. Card frames are neutral; rarity color stays in the emblem and label rather than the frame or its shadow.

 The earlier forest was too busy; muted teal was too empty. Jade stone established the accepted contrast and texture amount, then swirls replaced its rocks/puddles. `card-backdrops.json` is now the authoritative assignment: Boof Nitza Storm blue; Gon G'ah Steel blue; Ma'Zga Arcane teal; Tor Tza reuses Gon G'ah Steel blue; Vi'Zel reuses Boof Nitza Storm blue; Goh Loomb reuses Gon G'ah Steel blue; Baobaw reuses Boof Nitza Storm blue; Ko Pak Root olive; Chu Perk reuses Boof Nitza Storm blue; Ba Be'aga Soul cyan; Vee Scol reuses Gon G'ah Steel blue. Every evolution of a type uses that same background. Rarity remains in the badge and label, not the background or frame. Use the configured fallback for an unknown portrait. Historical studies remain for reference, not as the active direction.

Rarity emblems use common triangle, uncommon square, rare diamond, epic pentagon, legendary hexagon and mythic star. All have a dark outer contour around the rarity-colored fill for contrast on the bright backgrounds. Boss guide crests and formation cards use these same shapes. Card frames use a subtle neutral border and shadow without a rarity-colored strip or glow. Goh Loomb's attack effects and floating amounts are amber `#FFB23F`; Baobaw's are violet `#C084FC`. Their existing attack geometry, timing and behavior stay unchanged. World terrain colors and Bag item essence palettes are not part of this combat-color swap.
