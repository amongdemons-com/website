...
- **Permanent progression** - a permanent demon collection with Echo summoning and Soul-based training (`/collection`), an item bag (`/bag`), an account skill tree fed by level-up stat points (`/skill-tree`), daily quests and a daily reward (`/camp`), public hunter profiles (`/hunter/:username`), and leaderboards (`/rankings`).
...
| `/bag` | Stackable items; Demon Echoes live in Collection |
...
| `GET` | `/collection/bootstrap` | Consolidated player, collection, and Echo progress |

### Bag

| Method | Route | Description |
| --- | --- | --- |
| `GET` | `/bag` | Read owned non-Echo item stacks |
| `POST` | `/collection/echoes/summon` | Consume exact Echoes and summon the permanent minimum-stat demon |
...
### Echoes, Summoning, and Collection
...
Existing permanent demons and their training remain untouched. Natural discovery remains recorded after Echoes are spent. The `20260912_collection_echoes_v1` schema migration retains up to one summon requirement for each unowned variant and converts all excess Echoes (all Echoes for owned variants) into Souls at Crowley’s per-Echo prices: Common 10, Uncommon 30, Rare 100, Epic 300, Legendary 1,000, Mythic 5,000. Quantity reduction and Soul credit commit together per player; retries cannot pay twice. Account merges apply the same rules. Preview compensation without writes using `node scripts/preview-echo-conversion.js`. Normal server startup or `npm run db:migrate` applies it; deploy with old server workers stopped so they cannot continue awarding uncapped Echoes.
...
| `player_bag` | Item stacks and retained internal Echo progress; Echo rows are excluded from Bag responses |
...
| `public/app/js/bag-ui.js` | Bag grid, sorting, and item details |
...
| `public/api/lib/echo-config.js` | Authoritative Echo keys, summoning thresholds, and merchant Soul prices |
